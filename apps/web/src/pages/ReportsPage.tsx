...
export function ReportsPage() {
  ...
  const trendData = isMonthly ? data.monthlyComparison : data.yearlyComparison;
  const categoryData = isMonthly ? data.categoryTotals : data.yearlyCategoryTotals;
  const barData = isMonthly ? data.budgetVsActual : data.monthlyBreakdown;
  ...
  return (
    <div className="space-y-6">
      <div className="card p-5 sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            <label className="flex flex-col gap-2 text-sm text-slate-500">
              Report scope
              <select
                className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-ink"
                value={reportScope}
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {summaryCards.map((item) => (
            <div key={item.label} className="rounded-2xl bg-mist px-4 py-4">
      ...
      <div className="card p-5 sm:p-6">
        <div className="section-title">{isMonthly ? "Monthly trend" : "Yearly trend"}</div>
        <div className="mt-1 text-sm text-slate-500">
          {isMonthly
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <div className="card p-5 sm:p-6">
          <div className="section-title">{isMonthly ? "Expense distribution" : "Yearly category mix"}</div>
          <div className="mt-1 text-sm text-slate-500">
            {isMonthly
      ...
      <div className="card p-5 sm:p-6">
        ...
        <div className="mt-4 space-y-3">
          {categoryData.length === 0 ? (
            <div className="rounded-2xl bg-mist px-4 py-6 text-sm text-slate-500">
              {isMonthly ? "No expense transactions found for this month." : "No expense transactions found for this year."}
            </div>
          ) : (
            categoryData.map((item: any) => (
              <div key={item.category} className="flex items-center justify-between gap-4 rounded-2xl bg-mist px-4 py-3">
                <div className="flex items-center gap-3">
                  <span className="h-3 w-3 rounded-full" style={{ backgroundColor: item.color }} />
                  <span className="font-medium text-ink">{item.category}</span>
                </div>
                <span className="font-medium text-ink">{formatCurrency(Number(item.total))}</span>
              </div>
            ))
          )}
        </div>
